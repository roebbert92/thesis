...
def train_model(corpus: Corpus,
    ...
    if not os.path.exists(last_ckpt_path) and not os.path.exists(
            best_ckpt_path):
        ...
        if finetuning == "full":
            ...
            gc.collect()
            torch.cuda.empty_cache()
        elif finetuning == "no" and ckpt_path is not None:
            ckpt_dir_path = os.path.dirname(ckpt_path)
            for file_name in ["best-model.pt", "final-model.pt"]:
                shutil.copy(os.path.join(ckpt_dir_path, file_name),
                            os.path.join(model_output_path, file_name))

    # metrics
    # get metrics for last ckpt
    while not os.path.exists(last_ckpt_path):
        time.sleep(5)
    ...
    for seed in seeds:
        ...
        while not trained:
            try:
                ...
                if os.path.join(
                        model_output_path,
                        "final-model.pt") not in ckpt_files and os.path.join(
                            model_output_path,
                            "best-model.pt") not in ckpt_files:
        ...
        for gaz, finetuning, pretrained, config in configs:
            ...
            while not trained:
                try:
                    ...
                    if os.path.join(model_output_path, "final-model.pt"
                                    ) not in ckpt_files and os.path.join(
                                        model_output_path,
                                        "best-model.pt") not in ckpt_files:
                        for ckpt_file in ckpt_files:
                            os.remove(ckpt_file)
