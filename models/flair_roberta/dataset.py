...
from data_preprocessing.tokenize import handle_results, MENTION_START, MENTION_END, get_subtokens
...
class BIONERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        token_sequence, label_sequence, doc_id = data_item["tokens"], data_item["label_sequence"], data_item["doc_id"]
        label_ids = [
            self.label_to_idx[label_item] for label_item in label_sequence
        ]

        concat_word = " "
        token_sequence = concat_word.join(
            token_sequence[:min(self.max_length - 2, len(token_sequence))])
        label_ids = label_ids[:min(self.max_length -
                                             2, len(label_ids))]
        # convert string to ids
        input_tokens = [get_subtokens(self.tokenizer, x) for idx, x in enumerate(token_sequence.split(concat_word))]
        subword_map = [token_idx for token_idx, token_ids in enumerate(input_tokens) for _ in token_ids]

        # append search results with [EOS] token
        processed_input = [self.tokenizer.cls_token] + [token for tokens in input_tokens for token in tokens] + [self.tokenizer.eos_token]
        handle_results(self.tokenizer, processed_input, search_result, self.sent_use_labels, self.sent_use_mentions, self.gaz_use_labels, self.gaz_use_mentions)

        assert len(processed_input) <= self.max_length # type: ignore
        input_ids = torch.tensor(self.tokenizer.convert_tokens_to_ids(processed_input), dtype=torch.long)
    ...
    def _read_json(input_file):
        ...
        for item in items:
            label_sequence = []
            entity_range = {(ent["start"]+span): ent_idx for ent_idx, ent in enumerate(item["entities"]) for span in range(ent["end"]-ent["start"])}
