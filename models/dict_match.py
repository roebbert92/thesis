...
def experiment01(gazetteer_name: str, gazetteer_paths: List[str]):
    seeds = [1, 2, 3]
    config = {
        "name": f"dict_match_{gazetteer_name}",
    ...
    for seed in seeds:
        ...
        trainer.test(model, model.get_dataloader(files["train"], config["batch_size"]))
        save_metrics("lowner_train")
        trainer.test(model, model.get_dataloader(files["dev"], config["batch_size"]))
        save_metrics("lowner_dev")
        trainer.test(model, model.get_dataloader(files["test"], config["batch_size"]))
...
def experiment_03():
    ...
    content_to_combination = {
        ...
        for key, value in combination_to_content.items()
        for comb in value
    }
    for comb_idx, database_combinations in enumerate(gazetteer_content):
        for db_idx, database_comb in enumerate(database_combinations):
            ...
            with open(tmp_path, "w") as file:
                json.dump(
                    gazetteers[content_to_combination[f"{comb_idx}_{db_idx}"]], file
                )
            ...
            for seed in seeds:
                ...
                for part in ["train", "dev", "test"]:
                    ...
                    trainer.test(
                        model, model.get_dataloader(files[part], config["batch_size"])
                    )
                    save_metrics(part)
            os.remove(tmp_path)
...
if __name__ == "__main__":
    experiment01(
        "sent", [os.path.join(thesis_path, "data/multiconer/multiconer_sent.json")]
    )
    experiment01(
        "gaz", [os.path.join(thesis_path, "data/multiconer/multiconer_gaz.json")]
    )
    experiment01(
        "lownergaz",
    ...
    experiment01(
        "lownergaz_gaz", [os.path.join(thesis_path, "data/mlowner/lownergaz_gaz.json")]
    )
    experiment01(
        "lownergaz_sent",
        [
