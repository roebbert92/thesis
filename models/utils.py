...
def batch_select(tensor, idx, device=None):
    ...
    if tensor.shape[
            -1] == 1:  # If selected element is scalar, restore original dim
...
def batched_masked_select(tensor: torch.FloatTensor, mask: torch.Tensor):
    ...
    mask_sorted, indices = torch.sort(mask.long(),
                                      descending=True,
                                      stable=True,
                                      dim=-1)
    mask_sorted = mask_sorted.bool()

    return dim_batched_index_select(
        tensor, indices)[:, :max_len], mask_sorted[:, :max_len]
...
def dim_batched_index_select(
    ...
    target = target.reshape(math.prod([*target_size[:dim]]),
                            *target_size[dim:])
    indices = indices.view(math.prod([*indices_size[:dim]]),
                           *indices_size[dim:])
...
def make_linear(in_features, out_features, bias=True, std=0.02):
    ...
    device = torch.cuda.device_count() - 1 if torch.cuda.is_available(
    ) else "cpu"
    linear = nn.Linear(in_features, out_features, bias, device=device)
...
def make_ffnn(feat_size,
              hidden_size,
              output_size,
              dropout,
              std=0.02,
              activation='relu'):
    if hidden_size is None or hidden_size == 0 or hidden_size == [] or hidden_size == [
            0
    ]:
    ...
    ffnn = [
        make_linear(feat_size, hidden_size[0], std=std), ACT2FN[activation],
        dropout
    ]
    for i in range(1, len(hidden_size)):
        ffnn += [
            make_linear(hidden_size[i - 1], hidden_size[i], std=std),
            ACT2FN[activation], dropout
        ]
...
def prepare_pair_embeddings(col_vecs: torch.FloatTensor,
    ...
    if len(row_vecs.size()) == 2:  # no beam size
        return torch.cat([
            ...
            row_vecs.unsqueeze(1).expand(-1, col_vecs.size(0), -1)
        ],
                         dim=-1)
    else:
        return torch.cat([
            col_vecs.unsqueeze(1).expand(-1, row_vecs.size(1), -1, -1),
            row_vecs.unsqueeze(2).expand(-1, -1, col_vecs.size(1), -1)
        ],
                         dim=-1)
