...
class NERRunner(Runner):
    def evaluate(self,
        ...
        util.runner.logger.info(
            'Step %d: evaluating on %d samples with batch_size %d' %
            (step, len(tensor_examples), eval_batch_size))

        evalloader = DataLoader(tensor_examples,
        ...
        for i, (doc_keys, tensor_example) in enumerate(evalloader):
            ...
            for batch_id, doc_key in enumerate(doc_keys):
                ...
                decoded_results.update(gold_res)
                evaluator.update(**decoded_results)
                if predict:
...
if __name__ == '__main__':
    ...
    runner = NERRunner(config_file="configs/ner.conf",
                       config_name=config_name,
                       gpu_id=gpu_id)

    if saved_suffix is not None:
        model, start_epoch = runner.initialize_model(saved_suffix,
