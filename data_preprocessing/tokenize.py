...
def map_entities_to_sentence(
    entities, sentence, inv_subtoken_map, subtoken_map, label_to_id
):
    if len(entities) > 0:
...
def get_target_sentence(tokenizer: PreTrainedTokenizer, label_to_id, tokens, entities):
    ...
    target_sentence, ent_indices, ent_type_sequence, _ = map_entities_to_sentence(
        [ent for ent in entities if ent.get("error", -1) != 2],
        processed,
        inv_subtoken_map,
        subtoken_map,
        label_to_id,
    )
    return (processed, target_sentence, ent_type_sequence, ent_indices, subtoken_map)


def get_input_sentence(tokenizer: PreTrainedTokenizer, doc, insert_prefix=True):
...
def tokenize_json(
    ...
    for inst_id, instance in tqdm(
        enumerate(instances), desc="Tokenization", total=len(instances)
    ):
        tokens = instance["tokens"]
        entities = instance["entities"]
        extended = instance["extended"]
        doc_id = (
            instance["doc_id"] if "doc_id" in instance else name + "_" + str(inst_id)
        )
        ...
        (
            ...
            entity_indices,
            subtoken_map,
        ) = get_target_sentence(tokenizer, label_to_id, tokens, entities)

        # insert prefix (instruction for model) here
        input_sentence = get_input_sentence(
            tokenizer, extended, prepend_task_description
        )
...
def handle_results(
    ...
    for result in results:
        ...
        if meta["data_type"] == "gazetteers":
            ...
            entity_ends = {entity["end"]: entity for entity in meta["entities"]}
                ...
                if word_idx in entity_ends:
                    ...
                    if sent_use_labels:
                        ...
                        processed_doc.extend(
                            tokenized_type[1:] if sent_use_mentions else tokenized_type
                        )
                # entity start
...
def cosine_sim(a, b):
    ...
    np_array = np.sum(same_size_a * b, axis=1) / (
        norm(same_size_a, axis=1) * norm(b, axis=1)
    )
...
def get_input_sentence_database_with_filter(
    ...
    results = database.run(
        query=sentence,
        params={"filters": {"$and": {"$not": exclude_filter, **filters}}},
    )
    ...
    if len(results) > 0 and cosine_model is not None:
        if results[0].embedding is not None:
            ...
            embeds = np.asarray(
                [
                    get_embedding(cosine_model, embed_cache, sent)
                    for sent in [sentence, *[" ".join(res.content) for res in results]]
                ]
            )
...
def tokenize_database_json_with_filter(
    ...
    for inst_id, instance in tqdm(
        enumerate(instances), desc="Tokenization with DB", total=len(instances)
    ):
        tokens = instance["tokens"]
        entities = instance["entities"]
        extended = instance["extended"]
...
def get_input_sentence_database(
    tokenizer: PreTrainedTokenizer,
    doc,
...
def init_query_filter_db(
    ...
    search_pipeline = search(**search_kwargs)
    items = instances
...
def run_database_query_with_filter(chunk_idx: int):
    ...
    results = []
    for item in chunk:
        sentence = " ".join(item["extended"])
        params = None
        if exact_match_filter:
            params = {"filters": {"$and": {"$not": {"doc_id": item["doc_id"]}}}}
        result = search_pipeline.run(query=sentence, params=params)  # type: Pipeline
        results.append(result["documents"] if result is not None else [])
    return chunk_idx, results


def query_database_with_filter(
    instances: List, search_func, filter_exact_match: bool = False, **search_kwargs
):
    chunk_size = 50
    cpu_count = 4
    with tqdm(desc="Querying database", total=len(instances)) as pbar:
        with mp.get_context("spawn").Pool(
            ...
            for chunk_idx, results in pool.map(
                run_database_query_with_filter, chunks, len(chunks) // cpu_count
            ):
...
def query_database(instances: List, search: Pipeline):
    ...
    with tqdm(desc="Querying database", total=len(instances)) as pbar:
        for i in range(0, len(instances), chunk_size):
            ...
            for j, res in zip(
                range(len(chunk)), results["documents"] if results is not None else []
            ):
                pbar.update(1)
                yield j + i, res


def tokenize_database_json(
    tokenizer: PreTrainedTokenizer,
    file_name,
    type_file,
    search: Pipeline,
    ...
    name = os.path.basename(os.path.splitext(file_name)[0])
    for instance_idx, search_result in tqdm(
        query_database(instances, search),
...
def tokenize_search_results_json(
    tokenizer: PreTrainedTokenizer,
    file_name: str,
    type_file: str,
    ...
    name = os.path.basename(os.path.splitext(file_name)[0])
    for instance_idx, instance in tqdm(
        enumerate(instances), desc="Tokenization with DB", total=len(instances)
    ):
        tokens = instance["tokens"]
        entities = instance["entities"]
