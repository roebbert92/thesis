...
def map_entities_to_sentence(
    ...
    sorted_pos = sorted(
        [(inv_subtoken_map[x][0], MENTION_END, label_to_id[t], idx) for idx, (x, t) in enumerate(zip(m_endings, m_types))] +
...
def get_target_sentence(tokenizer: PreTrainedTokenizer, label_to_id, tokens, entities):
    ...
    for word_idx, word in enumerate(tokens):
        ...
        inv_subtoken_map[word_idx] = (
            len(processed), len(processed) + len(subtokens))
...
def tokenize_json(tokenizer: PreTrainedTokenizer,
    ...
    for file_name in [train_file, dev_file, test_file]:
        ...
        with open(f"{os.path.splitext(file_name)[0]}.{os.path.basename(os.path.splitext(tokenizer.name_or_path)[0])}.jsonlines", "w", encoding="utf-8") as output_file:
