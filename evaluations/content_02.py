...
from data_metrics.search_sample_similarity import (
    SearchSampleSimilarity,
    get_search_sample_similarity_for_model,
    SearchSampleSimilarityCollator,
)
...
def process_metrics_file(metrics_filepath):
    ...
    metrics_per_sample = metrics.metrics_per_sample(dataset, types).to_dict(
        orient="records"
    )
    ...
    tmp_filepath = os.path.join(
        thesis_path, "evaluations", "metrics", "tmp", f"{uuid4()}.parquet"
    )
...
def get_per_sample_metrics():
    metrics_file_path = os.path.join(
        thesis_path, "evaluations", "metrics", "02_content_per_sample_metrics.parquet"
    )
    if not os.path.exists(metrics_file_path):
        ...
        for seed, size, error_ratio, error_part, timestep, part in product(
            ...
            if timestep == 0:
                if error_part in ["train", "both"] and part in ["train", "dev"]:
                    ...
                    ] = os.path.join(labeled_data_path, f"error_lowner_{part}.json")
                else:
                    datasets[
                        f"{seed}_{size}_{error_ratio}_{error_part}_{timestep}_{part}"
                    ] = os.path.join(
            ...
            if timestep > 0:
                datasets[
                    f"{seed}_{size}_{error_ratio}_{error_part}_{timestep}_{part}"
                ] = os.path.join(thesis_path, "data", "mlowner", f"lowner_{part}.json")
        ...
        metrics_files = [
            ...
            if os.path.splitext(fp.split(os.path.sep)[-1])[0].split("_")[0] == "last"
        ]
        with mp.Pool(
            mp.cpu_count() // 2,
        ...
        if pqwriter is not None:
            pqwriter.close()
            shutil.rmtree(os.path.join(thesis_path, "evaluations", "metrics", "tmp"))
    ...
    for seed, size, error_ratio, error_part, timestep, part in product(
        ...
        if timestep == 2:
            labeled_data[
                ...
                os.path.join(
                    thesis_path, "data", "multiconer", "filtered_multiconer.json"
                ),
            ]
        #  process datasets
        ...
        if timestep == 0:
            if error_part in ["train", "both"] and part in ["train", "dev"]:
                datasets[
                    f"{seed}_{size}_{error_ratio}_{error_part}_{timestep}_{part}"
                ] = os.path.join(datasets_path, f"error_lowner_{part}.json")
            else:
                datasets[
                    f"{seed}_{size}_{error_ratio}_{error_part}_{timestep}_{part}"
                ] = os.path.join(thesis_path, "data", "mlowner", f"lowner_{part}.json")
...
def get_labeled_data_entity_coverage():
    ...
    if os.path.exists(eecr_labeled_data_data_path):
        ...
        pqwriter = None
        os.makedirs(os.path.dirname(eecr_labeled_data_data_path), exist_ok=True)
        labeled_data, datasets = get_labeled_data()
        with mp.Pool(mp.cpu_count() - 5) as pool:
            with tqdm(
                total=len(labeled_data), desc="Compute EECR labeled data"
            ) as pbar:
...
def get_ecr_per_sample_labeled_data_df(
    key, labeled_data_paths: List[str], dataset_path
):
    seed, size, error_ratio, error_part, timestep, part = key.split("_")
    labeled_data = []
    for labeled_data_path in labeled_data_paths:
...
def get_search_results_data():
    ...
    for seed, size, error_ratio, error_part, timestep, part in product(
        ...
        if timestep == 1:
            search_results[
                f"{seed}_{size}_{error_ratio}_{error_part}_{timestep}_{part}"
            ] = os.path.join(search_base_path, f"sampled_search_results_{part}.pkl")
...
def get_search_results_entity_coverage_per_sample():
    ...
    if os.path.exists(eecr_search_results_data_path):
        ...
        with mp.Pool(mp.cpu_count() - 1) as pool:
            with tqdm(
                total=len(search_results), desc="Compute EECR search results"
            ) as pbar:
                results = pool.starmap_async(
...
def get_ccr_per_sample_df(search_result_path, labeled_data_path):
    ...
    for ccr in get_search_sample_similarity_for_model(model, collator, dataset, search):
...
def get_search_results_data_ccr_metrics():
    ...
    if os.path.exists(metrics_file_path):
        ...
        with mp.get_context("spawn").Pool(1, get_ccr_init) as pool:
            global pbar
            with tqdm(
                total=len(search_result_combs), desc="CCR search results"
            ) as pbar:
                results = pool.starmap_async(
