...
from data_metrics.entity_coverage_ratio import (
    calc_ecr_classes,
    entity_coverage_ratio,
    entity_coverage_ratio_precounted,
    count_entities,
)
...
def process_metrics_file(metrics_filepath):
    ...
    model = fp[-2]
    checkpoint = fp[-1][:4]
    for sample_metrics in metrics.metrics_per_sample(datasets[dataset], types).to_dict(
        orient="records"
    ):
        metrics_list.append(
            {
...
def get_per_sample_metrics():
    metrics_file_path = os.path.join(
        ...
        "evaluations",
        "metrics",
        "01_performance_per_sample_metrics.parquet",
    )
    if not os.path.exists(metrics_file_path):
        ...
        metrics_df.to_parquet(metrics_file_path)
    else:
        metrics_df = pd.read_parquet(metrics_file_path)
    return metrics_df


def aggregate_error_rate(metrics_df: pd.DataFrame, checkpoint: str, dataset: str):
...
def get_micro_f1_precision_recall(
    metrics_df: pd.DataFrame, checkpoint: str, dataset: str
):
    agg_df = aggregate_performance_metrics(metrics_df)

    avg_df = agg_df.pivot_table(
        index=["model", "checkpoint", "dataset"],
        values=["f1", "precision", "recall"],
        aggfunc=("mean", "std"),
    ).reset_index()

    return avg_df[(avg_df["checkpoint"] == checkpoint) & (avg_df["dataset"] == dataset)]
...
def get_precision(metrics_df: pd.DataFrame, checkpoint: str, dataset: str):
    ...
    return avg_precision[
        (avg_precision["checkpoint"] == checkpoint)
...
def get_recall(metrics_df: pd.DataFrame, checkpoint: str, dataset: str):
    ...
    return avg_recall[
        (avg_recall["checkpoint"] == checkpoint) & (avg_recall["dataset"] == dataset)
    ]
...
def get_micro_f1(metrics_df: pd.DataFrame, checkpoint: str, dataset: str):
    ...
    avg_f1 = agg_df.pivot_table(
        index=["model", "checkpoint", "dataset"], values=["f1"], aggfunc=("mean", "std")
    ).reset_index()

    return avg_f1[(avg_f1["checkpoint"] == checkpoint) & (avg_f1["dataset"] == dataset)]
...
def get_error_types(metrics_df: pd.DataFrame, checkpoint: str, dataset: str):
    ...
    return avg_errors[
        (avg_errors["checkpoint"] == checkpoint) & (avg_errors["dataset"] == dataset)
    ]
...
def get_f1_per_entity(metrics_df: pd.DataFrame, checkpoint: str, dataset: str):
    ...
    avg_f1 = (
        ...
        .swaplevel(axis=1, i=2, j=0)
        .sort_index(axis=1, level=2, sort_remaining=True)
        .reset_index()
    )
...
def get_error_types_per_entity(metrics_df: pd.DataFrame, checkpoint: str, dataset: str):
    error_types = [
        "error_type1",
        "error_type2",
        "error_type3",
...
def get_labeled_data():
    labeled_data_path = os.path.join(
        thesis_path, "evaluations", "metrics", "01_performance_labeled_data.pkl.tar.gz"
    )
    if os.path.exists(labeled_data_path):
        ...
        for key in ["gaz", "lownergaz", "sent", "lownergaz_gaz"]:
            if key in ["gaz", "sent"]:
                export_path = os.path.join(
                    thesis_path, "data", "multiconer", f"multiconer_{key}.json"
                )
            elif key == "lownergaz":
                export_path = os.path.join(
                    thesis_path, "data", "mlowner", "lowner_gazetteer_db.json"
                )
            else:
                export_path = os.path.join(
                    thesis_path, "data", "mlowner", "lownergaz_gaz.json"
                )
        ...
        for key in ["lowner_train", "lowner_dev", "lowner_test"]:
            with open(
                os.path.join(thesis_path, "data", "mlowner", f"{key}.json"),
                "r",
                encoding="utf-8",
            ) as file:
...
def get_labeled_data_entity_coverage():
    ...
    if os.path.exists(eecr_labeled_data_path):
        ...
        eecr_metrics = []
        with tqdm(
            total=len(models_to_labeled_data) * len(dataset), desc="EECR labeled data"
        ) as pbar:
                    ...
                    eecr_metrics.append(
                        {
                            ...
                            "eecr": eecr,
                            **{key: len(value) for key, value in ecr_classes.items()},
                        }
                    )
                    pbar.update(1)
...
def get_labeled_data_entity_coverage_per_sample():
    ...
    if os.path.exists(eecr_labeled_data_per_sample_path):
        ...
        os.makedirs(os.path.dirname(eecr_labeled_data_per_sample_path), exist_ok=True)
        labeled_data, models_to_labeled_data, dataset = get_labeled_data()
        eecr_metrics = []
        with tqdm(
            total=len(models_to_labeled_data) * len(dataset),
            desc="EECR labeled data per sample",
        ) as pbar:
            for model, data_parts in models_to_labeled_data.items():
                data = [item for part in data_parts for item in labeled_data[part]]
...
def get_search_results_entity_coverage_per_sample():
    ...
    if os.path.exists(eecr_search_results_data_path):
        ...
        os.makedirs(os.path.dirname(eecr_search_results_data_path), exist_ok=True)
        search_results, dataset = get_search_results_data()
        eecr_metrics = []
        with tqdm(
            total=len(search_results) * len(dataset), desc="EECR search results"
        ) as pbar:
...
def aggregate_per_sample_eecr_metrics(metrics_df: pd.DataFrame):
    return (
        metrics_df.pivot_table(
            values=["eecr"], index=["model", "dataset"], aggfunc=["mean", "std"]
        )
        .swaplevel(0, 1, 1)
        .reset_index()
    )
...
def aggregate_per_sample_ecr_classes(metrics_df: pd.DataFrame):
    ...
    return metrics_df.pivot_table(
        values=ECR_CLASSES_ORDER, index=["model", "dataset"], aggfunc="sum"
    )[ECR_CLASSES_ORDER].reset_index()
...
def get_entity_coverages(dataset: str):
    ...
    eecr_table = (
        labeled_data_eecr_df[["model", "dataset", "eecr"]]
        .set_index(["model", "dataset"])
        .join(
            agg_labeled_data_eecr_sample_df.set_index(["model", "dataset"]),
            on=["model", "dataset"],
            lsuffix="_labeled_data",
        )
        .join(
            agg_search_results_data_eecr.set_index(["model", "dataset"]),
            on=["model", "dataset"],
            lsuffix="_labeled_data_per_sample",
            rsuffix="_search_results",
        )
        .reset_index()
    )
...
def get_ecr_plotable_table(ecr_df: pd.DataFrame, dataset: str):
    plt_ecr_df = ecr_df[ecr_df["dataset"] == dataset].loc[
...
def get_search_results_data_ccr_metrics():
    ...
    if os.path.exists(metrics_file_path):
        ...
        # ]
        with tqdm(
            total=len(search_results) * len(dataset), desc="CCR search results"
        ) as pbar:
            for model, search_result in search_results.items():
                for dataset_name, search in search_result.items():
                    for ccr in get_search_sample_similarity(
                        ...
                        ccr_metrics.append(
                            {"model": model, "dataset": dataset_name, **ccr}
                        )
                    pbar.update(1)
...
def get_search_results_data_ccr_max():
    ...
    return ccr_metrics_df.pivot_table(
        values="max", index=["model", "dataset"], aggfunc=["mean", "std"]
    ).reset_index()
