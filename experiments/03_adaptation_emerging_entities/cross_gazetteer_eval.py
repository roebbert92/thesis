...
eval_gazetteer_contents = {
    2: [("wnut_train", "wnut_dev"),
        ("lownergaz_sent", "wnut_train", "wnut_dev"), ("lownergaz_sent", )],
    3: [("wnut_train", "wnut_dev", "wnut_test"),
...
def test_model(config, best_ckpt_path, last_ckpt_path, dataset: Dataset, name):
    metrics_base_path = os.path.join(
        config["data_path"], f"seed_{str(seed)}", "05_cross_gazetteer_eval",
...
for timestep, eval_gaz in eval_gazetteer_contents.items():
    wnut_only = NERDataProcessor(
        ...
        os.path.join(config["data_path"], "02_tokenized_dataset",
                     ...
                     f"1_{timestep}", "wnut_train.t5-small.jsonlines"),
        os.path.join(config["data_path"], "02_tokenized_dataset",
                     "ann_6_bm25_12_reciprocal_rank_fusion_16",
                     f"1_{timestep}", "wnut_dev.t5-small.jsonlines"),
        os.path.join(config["data_path"], "02_tokenized_dataset",
                     "ann_6_bm25_12_reciprocal_rank_fusion_16",
                     f"1_{timestep}", "wnut_test.t5-small.jsonlines"),
        files["types"],
        use_cache=False)
    ...
    lownergaz_wnut = NERDataProcessor(
        ...
        os.path.join(config["data_path"], "02_tokenized_dataset",
                     "ann_6_bm25_12_reciprocal_rank_fusion_16",
                     f"2_{timestep}", "wnut_train.t5-small.jsonlines"),
        os.path.join(config["data_path"], "02_tokenized_dataset",
                     "ann_6_bm25_12_reciprocal_rank_fusion_16",
                     f"2_{timestep}", "wnut_dev.t5-small.jsonlines"),
        os.path.join(config["data_path"], "02_tokenized_dataset",
                     "ann_6_bm25_12_reciprocal_rank_fusion_16",
                     f"2_{timestep}", "wnut_test.t5-small.jsonlines"),
        files["types"],
        use_cache=False)
    ...
    for seed in seeds:
        for train_gaz_idx, train_gaz in enumerate(train_gazetteer_contents):
            ...
            for eval_gaz_idx, _ in enumerate(eval_gaz):
                if eval_gaz_idx == 0:
                    train_dataset, dev_dataset, test_dataset = wnut_only.get_tensor_samples(
                    )
                elif eval_gaz_idx == 1:
                    train_dataset, dev_dataset, test_dataset = lownergaz_wnut.get_tensor_samples(
                    )
                else:
                    train_dataset, dev_dataset, test_dataset = lownergaz_only.get_tensor_samples(
                    )
