...
datasets = {"wnut_train": wnut_train, "wnut_dev": wnut_dev, "wnut_test": wnut_test}

total = [
    (seed, gaz, finetuning, pretrained, config)
    for seed, (gaz, finetuning, pretrained, config) in product(seeds, configs)
    if gaz
]
already_computed = []
...
def train_model(
    seed: int, config: dict, tokenized_files: dict, ckpt_path: Optional[str]
):
    ...
    def get_model_trainer():
        if ckpt_path is not None:
            model = ASPT5Model.load_from_checkpoint(
                ckpt_path, config=train_config, tokenizer=tokenizer
            )
        else:
    ...
    while not trained:
        try:
            ...
            train_loader, dev_val_loader = get_dataloaders()
            model, trainer = get_model_trainer()
    return os.path.join(checkpoint_base_path, "last.ckpt"), os.path.join(
        checkpoint_base_path, "best.ckpt"
    )
...
if __name__ == "__main__":
        ...
        if "lownergaz_sent" in database_comb and len(database_comb) > 1:
            ...
            if not lownergaz_exists or not sent_exists:
                ...
                if not lownergaz_exists:
                    ...
                    elasticsearch_client.indices.clone(
                        index="lownergaz", target=name + "_lownergaz"
                    )
                    elasticsearch_client.indices.put_settings(
                        index=name + "_lownergaz",
                ...
                if not sent_exists:
                    # copy sent database
                    elasticsearch_client.indices.put_settings(
                        index="sent", body={"index": {"blocks": {"write": True}}}
                    )
                    elasticsearch_client.indices.clone(
                        index="sent", target=name + "_sent"
                    )
                    elasticsearch_client.indices.put_settings(
                        index=name + "_sent",
                        body={"index": {"blocks": {"write": False}}},
            ...
            if not elasticsearch_client.indices.exists(
                index=name + "_lownergaz"
            ) or not elasticsearch_client.indices.exists(index=name + "_sent"):
                dataset = []
    ...
    search_configs = set(
        [
            ...
            else ("None", "None", "None", "None", "None", "None", config["data_path"])
            for _, _, _, config in configs
            if config is not None
        ]
    )
    for search_config in search_configs:
        ...
        if search_join_method == "None":
            tokenized_data_path = os.path.join(
                data_path, "02_tokenized_dataset", search_config_name
            )
            ...
            for comb_idx, database_combinations in enumerate(gazetteer_content):
                for db_idx, database_comb in enumerate(database_combinations):
                    ...
                    def get_search():
                        if len(database_comb) > 1 or "wnut_train" in database_comb:
                            name = "_".join(database_comb)
                            search = setup_database(
                                sent_search_algorithm,
                                sent_search_topk,
                    ...
                    os.makedirs(tokenized_data_path, exist_ok=True)
                    tokenized_files[search_config_name][f"{comb_idx}_{db_idx}"] = {}
    ...
    for seed, gazetteer, finetuning, pretrained, config in total:
        ...
        if not gazetteer:
            ...
            test_model(config, best_ckpt_path, last_ckpt_path, train_dataset, "train")
                ...
                if last_ckpt_path == best_ckpt_path == "":
                    if finetuning == "no":
                        ...
                        last_ckpt_path, best_ckpt_path = train_model(
                            seed,
                            config,
                            tokenized_files[search_config_name][f"{comb_idx}_0"],
                            config["ckpt_path"] if "ckpt_path" in config else None,
                        )
                for db_idx, database_comb in enumerate(database_combinations):
                    processor = NERDataProcessor(
                        config,
                    ...
                    test_model(
                        config, best_ckpt_path, last_ckpt_path, train_dataset, "train"
                    )
                    test_model(
                        config, best_ckpt_path, last_ckpt_path, dev_dataset, "dev"
                    )
                    test_model(
                        config,
                        best_ckpt_path,
