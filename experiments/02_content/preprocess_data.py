...
def get_tokenized_filepath(
    config, files, file_path, search_results, data_path, output_name: Optional[str]
):
    ...
    return tokenize_search_results_json(
        tokenizer,
        file_path,
        files["types"],
        search_results,
...
def generate_experiment_data(
    seeds: List[int], gazetteer_sizes: List[int], error_percent_ratios: List[int]
):
    files = {
        ...
        "multiconer": os.path.join(
            thesis_path, "data", "multiconer", "multiconer_test.json"
        ),
        "lownergaz": os.path.join(
            thesis_path, "data", "mlowner", "lowner_gazetteer.json"
        ),
    }
    ...
    with mp.get_context("spawn").Pool(cpu_count // 2) as pool:
        for (
            ...
            error_percent_ratio,
            search_result_paths,
        ) in pool.starmap(get_search_results, search_results_configs):
            experiment_data_paths["01_search_results"][
                f"{seed}_{gazetteer_size}_{error_percent_ratio}"
            ] = search_result_paths
    ...
    for part in ["train", "dev", "test"]:
        file_path_part = os.path.join(search_base_path, f"search_results_{part}.pkl")
        if not os.path.exists(file_path_part):
            search_results = get_search_results_for_file(full_search, files[part])
...
def prep_tokenized_dataset(
    ...
    for part in ["train", "dev"]:
        with open(
            search_results_paths[f"error_search_results_error_{part}"], "rb"
        ) as file:
            error_search_results_error_part = pickle.load(file)
        tokenized_files[f"error_search_error_{part}"] = get_tokenized_filepath(
            config,
            files,
            dataset_paths[f"error_lowner_{part}"],
            error_search_results_error_part,
            tokenized_data_path,
            f"error_search_error_{part}",
        )
    for part in ["train", "dev", "test"]:
        with open(search_results_paths[f"error_search_results_{part}"], "rb") as file:
            error_search_results_part = pickle.load(file)
        tokenized_files[f"error_search_{part}"] = get_tokenized_filepath(
            config,
            files,
            files[part],
            error_search_results_part,
            tokenized_data_path,
            f"error_search_{part}",
        )

    # prep sampled clean lowner train, dev, test
    for part in ["train", "dev", "test"]:
        with open(search_results_paths[f"sampled_search_results_{part}"], "rb") as file:
            error_search_results_error_part = pickle.load(file)
        tokenized_files[f"sampled_search_{part}"] = get_tokenized_filepath(
            ...
            error_search_results_error_part,
            tokenized_data_path,
            f"sampled_search_{part}",
        )

    for part in ["train", "dev"]:
        with open(
            search_results_paths[f"sampled_search_results_error_{part}"], "rb"
        ) as file:
            error_search_results_error_part = pickle.load(file)
        tokenized_files[f"sampled_search_error_{part}"] = get_tokenized_filepath(
            config,
            files,
            dataset_paths[f"error_lowner_{part}"],
            error_search_results_error_part,
    ...
    for part in ["train", "dev", "test"]:
        with open(search_results_paths[f"full_search_result_{part}"], "rb") as file:
            error_search_results_error_part = pickle.load(file)
...
def get_search_results(
    ...
    search_results_paths = {}
    for part in ["train", "dev"]:
        search_results_paths[f"error_search_results_error_{part}"] = os.path.join(
    ...
    if any(
        ...
        for part in ["train", "dev"]:
            ...
            with open(
                search_results_paths[f"error_search_results_error_{part}"], "wb"
            ) as file:
        ...
        for part in ["train", "dev", "test"]:
            ...
            with open(
                search_results_paths[f"error_search_results_{part}"], "wb"
            ) as file:
    ...
    for part in ["train", "dev"]:
        search_results_paths[f"sampled_search_results_error_{part}"] = os.path.join(
            search_base_path, f"sampled_search_results_error_{part}.pkl"
        )
    if any(
        [
            not os.path.exists(search_results_paths[f"sampled_search_results_{part}"])
            for part in ["train", "dev", "test"]
        ]
    ) or any(
        [
        ...
        for part in ["train", "dev", "test"]:
            ...
            with open(
                search_results_paths[f"sampled_search_results_{part}"], "wb"
            ) as file:
        ...
        for part in ["train", "dev"]:
            ...
            with open(
                search_results_paths[f"sampled_search_results_error_{part}"], "wb"
            ) as file:
                pickle.dump(sampled_search_results_part, file)
...
def create_sampled_dataset(config, files, seed: int, gazetteer_size: int):
    ...
    if not os.path.exists(dataset_paths["clean_sampled_multiconer"]):
        ...
        with open(
            dataset_paths["clean_sampled_multiconer"], "w", encoding="utf-8"
        ) as file:
            json.dump(sampled_multiconer, file)
        print("finished sampling multiconer")
    else:
        with open(dataset_paths["clean_sampled_multiconer"], encoding="utf-8") as file:
            sampled_multiconer = json.load(file)
    ...
    if not os.path.exists(dataset_paths["clean_sampled_lownergaz"]):
        ...
        with open(
            dataset_paths["clean_sampled_lownergaz"], "w", encoding="utf-8"
        ) as file:
...
def create_erroneous_dataset(
    ...
    for part in ["train", "dev"]:
        ...
        if not os.path.exists(dataset_paths[f"error_lowner_{part}"]):
            ...
            error_lowner_part = make_erroneous_dataset(
                lowner_part, list(types), error_ratio
            )
            with open(
                dataset_paths[f"error_lowner_{part}"], "w", encoding="utf-8"
            ) as file:
    ...
    if not os.path.exists(dataset_paths["error_sampled_multiconer"]):
        ...
        with open(
            dataset_paths["error_sampled_multiconer"], "w", encoding="utf-8"
        ) as file:
    ...
    if not os.path.exists(dataset_paths["error_sampled_lownergaz"]):
        ...
        with open(
            dataset_paths["error_sampled_lownergaz"], "w", encoding="utf-8"
        ) as file:
