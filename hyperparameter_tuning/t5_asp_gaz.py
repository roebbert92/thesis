...
from search.gaz.setup import add_gaz_search_components, add_multiconer_gazetteers, create_gaz_faiss_document_store, train_update_gaz_faiss_index
...
from hyperparameter_tuning.utils import factors, get_gazetteers_from_documents, get_search_results
...
def flan_t5_asp_gaz_configs():
    config = FLAN_T5_BASE.copy()

    config["data_path"] = os.path.join(thesis_path, "hyperparameter_tuning",
                                       "tune")
    config["name"] = "flan-t5_asp_gaz"
    config["batch_size"] = 40
...
def setup_database(search_algorithm: str, search_topk: int, reset=False):
    ...
    add_gaz_search_components(search,
                              search_algorithm,
                              search_topk,
                              reset=reset)
