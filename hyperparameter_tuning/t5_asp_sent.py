...
from hyperparameter_tuning.training import factors, get_gazetteers_from_documents, get_search_results, get_sentences_from_documents
...
def t5_asp_sent_configs():
    ...
    config["search_topk"] = tune.randint(1, 15)
    config["seed"] = 42
...
def add_multiconer_sentences(doc_store: BaseDocumentStore):
    if doc_store.get_document_count() == 0:
        with open(os.path.join(thesis_path, "data", "multiconer",
            ...
            docs = json.load(file)
        documents = get_sentences_from_documents(docs, "multiconer_test")
